...
class TimestampedExpiringSecureTokenTest extends \PHPUnit_Framework_TestCase
{
    /**
    ...
    protected function setUp()
    {
        ...
        $delegate0 = new Aes256CbcSha256SecureToken($encryptionKey, $signingKey);
        $delegate1 = new ExpiringSecureToken($delegate0, new \DateInterval('PT2S'));
        $this->token = new TimestampedSecureToken($delegate1);
    }
     ...
     * @depends testEncode
     */
    public function testGetTimestamp($token)
    {
        $issued = $this->token->getTimestamp($token);
        $this->assertTrue($issued instanceof \DateTime);
        $this->assertEquals($issued, new \DateTime());
    }
